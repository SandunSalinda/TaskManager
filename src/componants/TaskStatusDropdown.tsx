  ...
  return (
    <div className="relative inline-block flex-shrink-0">
      <select
        ...
        disabled={updating}
        className={`
          appearance-none px-2.5 py-1.5 text-xs font-medium rounded-md border focus:outline-none focus:ring-2
          transition-all duration-200 cursor-pointer min-w-[100px] bg-white pr-7
          ${updating ? 'opacity-70 cursor-not-allowed' : ''}
          ${
            status === 'pending' ? 'text-yellow-700 border-yellow-200 bg-yellow-50 focus:ring-yellow-300 hover:border-yellow-300' :
            status === 'in-progress' ? 'text-blue-700 border-blue-200 bg-blue-50 focus:ring-blue-300 hover:border-blue-300' :
            'text-green-700 border-green-200 bg-green-50 focus:ring-green-300 hover:border-green-300'
          }
        `}
      >
