...
export default function TaskActions({ taskId, onTaskDeleted }: TaskActionsProps) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showError(errorData.error || 'Failed to delete task.');
      }
    } catch (err: unknown) {
      console.error('Error deleting task:', err);
      showError(err instanceof Error ? `An error occurred while deleting the task: ${err.message}` : 'An unknown error occurred while deleting the task.');
    } finally {
      setDeleting(false);
    }
  };
  ...
  return (
    <div className="flex justify-end space-x-2">
      <Link 
        ...
        className="inline-flex items-center px-2.5 py-1 text-xs font-medium text-blue-700 bg-blue-50 border border-blue-200 rounded-md hover:bg-blue-100 hover:border-blue-300 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1"
      >
      ...
      {/* Render modal using Portal to document.body to avoid conflicts */}
      {showDeleteModal && mounted && typeof document !== 'undefined' && 
